...
Shader "S_SpawnFloor"
{
	Properties
	{
		...
		[ASEBegin]_VoronoiScale("Voronoi Scale", Float) = 0
		_VoronoiSpeed("Voronoi Speed", Float) = 0
		_VoronoiTwirlStrength("Voronoi Twirl Strength", Float) = 1
		[HDR]_Color0("Color 0", Color) = (0.9150943,0.1347204,0.08201315,0)
		_VoronoiTwirlOffset("Voronoi Twirl Offset", Vector) = (0,0,0,0)
		_Clip("Clip", Range( 0 , 1)) = 0.3298699
		[ASEEnd]_RotationSpeed("Rotation Speed", Float) = 1
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 _Color0;
			float2 _VoronoiTwirlOffset;
			float _VoronoiScale;
			float _VoronoiSpeed;
			float _VoronoiTwirlStrength;
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float cos47 = cos( ( _RotationSpeed * _TimeParameters.x ) );
				float sin47 = sin( ( _RotationSpeed * _TimeParameters.x ) );
				float2 rotator47 = mul( appendResult44_g3 - float2( 0,0 ) , float2x2( cos47 , -sin47 , sin47 , cos47 )) + float2( 0,0 );
				float2 coords11 = rotator47 * _VoronoiScale;
				float2 id11 = 0;
				float2 uv11 = 0;
				float voroi11 = voronoi11( coords11, time11, id11, uv11, 0, voronoiSmoothId11 );
				float clampResult26 = clamp( ( ( 1.0 - appendResult23_g2.x ) * voroi11 ) , 0.0 , 1.0 );
				float4 temp_output_17_0 = ( _Color0 * ( pow( appendResult23_g2.x , 2.0 ) * clampResult26 ) );
				
				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;
				float3 Color = temp_output_17_0.rgb;
				float Alpha = temp_output_17_0.r;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN ) : SV_TARGET
			{
				...
				surfaceDescription.Alpha = temp_output_17_0.r;
...
ASEEND*/
//CHKSM=CDFA68F15915058093960FD3CA143995B249EEFE